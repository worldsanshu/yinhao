...
class KeyDerivation {
  ...
  static Future<(Uint8List, Uint8List)> pbkdf2(String pass, Uint8List salt, {int? iterations}) async {
    final algo = Pbkdf2(
  ...
  static Future<Uint8List> combineKDFs(Uint8List k1, Uint8List k2, Uint8List k3, Uint8List masterSalt) async {
  ...
  static Future<(Uint8List, Uint8List)> encryptAesGcm(Uint8List key32, Uint8List plaintext) async {
    final cipher = AesGcm.with256bits();
    final nonce = randomBytes(12);
    final box = await cipher.encrypt(plaintext, secretKey: SecretKey(key32), nonce: nonce);
    return (Uint8List.fromList(box.cipherText + box.mac.bytes), nonce);
  }

  static Future<Uint8List> decryptAesGcm(Uint8List key32, Uint8List ciphertextPlusMac, Uint8List nonce) async {
    final cipher = AesGcm.with256bits();
